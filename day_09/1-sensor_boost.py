def intcode(arr, code_position, relative_base=0):
    if arr[code_position] == 99:
        return arr

    opcode, parameters_mode = get_opcode(arr[code_position])
    parameters = get_parameters(arr, code_position, parameters_mode, opcode, relative_base)
    if opcode == 1:
    ...
    arr = intcode(arr, position, relative_base)
    return arr
...
def get_parameters(arr, code_position, parameters_mode, opcode, relative_base):
    parameters = list()
    if not parameters_mode:
        ...
        for x in range(len(parameters_mode)):
            ...
            elif parameters_mode[-x] == 2:
                parameters.append(arr[relative_base + code_position + x])
            else:
